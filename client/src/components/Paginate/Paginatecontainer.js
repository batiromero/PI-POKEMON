...
export const Paginatecontainer = ({ setPokemonsViews, pokemonViews }) => {
  ...
  const handleClick = (e) => {
    setPaged({
      page: e.target.value, // al avanzar en cada uno 
          ...
          {listArray &&
            listArray.map((el, i) => (
              <div key={i} className="div_list">
                <Paginated li={<li key={i} className={el === Paged.page ? "listed" : "list"}
                      value={el}
...
export default connect(mapStateToProps, { setPokemonsViews })(
  Paginatecontainer
);
